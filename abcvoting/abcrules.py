...
def __init_rules():
    __RULESINFO = [
        ...
        ("rule-x", "Rule X", "Rule X", compute_rule_x, ("standard",), (True, False)),
        (
    ...
    for _ in range(committeesize):
        ...
        if verbose >= 2:
            ...
            tied_cands = [
                ...
                if (c > next_cand and (additional_score_cand[c] == max(additional_score_cand)))
            ]
            if len(tied_cands) > 0:
...
def __revseq_thiele_resolute(profile, committeesize, scorefct_str, verbose):
    ...
    for _ in range(profile.num_cand - committeesize):
        ...
        # find smallest elements in marg_util_cand and return indices
        cands_to_remove = [
            cand for cand in range(profile.num_cand) if marg_util_cand[cand] == min(marg_util_cand)
        ]
...
def compute_greedy_monroe(profile, committeesize, algorithm="standard", resolute=True, verbose=0):
    ...
    if verbose >= 2:
        ...
        for cand, voters in assignment:
            print(" candidate " + profile.names[cand] + " assigned to: ", end="")
            output = ""
...
def __seqphragmen_resolute(
    profile, committeesize, verbose, start_load=None, partial_committee=None
):
...
def __seqphragmen_irresolute(profile, committeesize, start_load=None, partial_committee=None):
...
def compute_seqphragmen(
    ...
    if resolute:
        committees, comm_loads = __seqphragmen_resolute(profile, committeesize, verbose)
    else:
        committees, comm_loads = __seqphragmen_irresolute(profile, committeesize)
...
def compute_rule_x(
    ...
    start_budget = {v: Fraction(committeesize, len(profile)) for v, _ in enumerate(profile)}
    ...
    for _ in range(committeesize):
        ...
        for committee, budget in commbugdets:
            ...
            if len(min_q) > 0:  # one or more candidates are affordable
                next_cands = [c for c in min_q.keys() if min_q[c] == min(min_q.values())]
                ...
                if skip_phragmen_phase:
                    ...
                    for v in range(len(profile)):
                        start_load[v] = Fraction(committeesize, len(profile)) - budget[v]
...
def compute_phragmen_enestroem(
    ...
    start_budget = {i: Fraction(profile[i].weight) for i in range(num_voters)}
